...
export function DeliveryMap({ onDeliveryCalculated, initialAddress = "", branchLocation = null }: DeliveryMapProps) {
  ...
  useEffect(() => {
    ...
    scriptElement.onload = () => {
      ...
      const branchName = branchLocation?.name || config.name;
      const branchAddress = branchLocation?.address || `${config.address.street}, ${config.address.postalCode} ${config.address.city}`;
    ...
    const searchAddresses = async () => {
      ...
      try {
        const response = await fetch(
          `/nominatim/search?format=json&q=${encodeURIComponent(address)}&countrycodes=fi&limit=5&addressdetails=1`
        );
